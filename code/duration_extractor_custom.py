...
from hparam_custom import HPDurationExtractor as hp
from hparam_custom import HPStft, HPText
...
from utils.text import TextProcessorInd, TextProcessor
...
class ConvTextEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        # hp.alphabet_size
        #self.embedding = nn.Embedding(78, hp.channels, padding_idx=0)
...
class Collate:
    def __init__(self, device):
        ...
        self.text_proc = TextProcessorInd()
...
if __name__ == '__main__':
    ...
    logdir = os.path.join('/content/drive/MyDrive/SpeedySpeech', 'logs', time.strftime("%Y-%m-%dT%H-%M-%S") + '-' + args.name)
    if args.from_checkpoint:

        args.from_checkpoint = os.path.join('/content/drive/MyDrive/SpeedySpeech', args.from_checkpoint)
        ...
        pretrained_state_dict = {k: checkpoint['state_dict'][k] for k in state_dict_keys}
        # Delete embedding state dict
        del pretrained_state_dict['txt_encoder.embedding.weight']
        ...
        print('Missing state dict: {}'.format(missing_keys))
        print('Unxpected state dict: {}',format(unexpected_keys))
        print('\n')

        #for param in m.audio_encoder.parameters(): param.requires_grad = False
        #for param in m.audio_decoder.parameters(): param.requires_grad = False
...
#python code/duration_extractor.py --name _ --batch_size 16 --epochs 1 --from_checkpoint checkpoints/duration_extractor.pth